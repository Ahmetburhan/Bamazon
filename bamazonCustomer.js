...
var purchase = function () {
    ...
    prompt.get(productInfo, function (err, res) {
        ...
        connection.query('SELECT * FROM Products WHERE ItemID=?', productPurchased[0].itemID, function (err, res) {
            ...
            //if the stock quantity available is less than the amount wanted the user will be alerted that the product is out of stock
            if (res[0].StockQuantity < productPurchased[0].Quantity) {
                console.log(colors.red('That product is out of stock!'));
                connection.end();

                //otherwise if the stock amount available is more than requested total will be calculated
