...
var addInventory = function (err, res) {
    //this adds the variable that will prompt the information needed to replenish the stock 
    var addInvt = {
        properties: {
            inventoryID: {
                description: colors.green('What is the ID # of the product you want to add inventory for?')
            },
            inventoryAmount: {
    ...
    prompt.get(addInvt, function (err, res) {
        ...
        connection.query("UPDATE Products SET StockQuantity = (StockQuantity + ?) WHERE ItemID = ?;", [inventoryUpdate[0].inventoryAmount, inventoryUpdate[0].inventoryID], function (err, result) {
            ...
            if (err) console.log('ID you entered not found in the database ' + err);
...
var addNewProduct = function () {
    ...
    var newProduct = {
        properties: {
            ...
            newItemPrice: { description: colors.gray('Please enter the price of the item in the format of xxx.00') },
